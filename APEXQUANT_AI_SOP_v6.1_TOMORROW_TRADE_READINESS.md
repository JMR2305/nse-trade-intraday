# ApexQuant AI — Standard Operating Procedure v6.1
...
| **Kite LTP overlay** | ✅ Enabled — `kite_overlay_enabled: true`, `kite_verified: true`, `kite_session_verified: true` *(verified at SOP generation time 20:45 IST — **must be re-verified before market open**)* |
...
┌─────────────────────────────────────────────────────┐
│  READY WITH WARNINGS                                │
│  (conditional on 09:00–09:10 IST checks — see §15) │
│                                                     │
│  ✅ All safety controls active                      │
│  ✅ Bootstrap enabled, ₹15,000 cap                  │
│  ✅ No overnight carry — DRREDDY closed             │
│  ✅ EOD square-off confirmed working                │
│  ⚠️  Kite session must be re-verified at 09:00 IST  │
│  ⚠️  LTIM.NS unavailable (provider issue, ongoing)  │
│  ⚠️  Last scan from 15:26 IST (normal post-market)  │
└─────────────────────────────────────────────────────┘
```

**No code blockers for tomorrow's trade session.** The LTIM warning is a pre-existing NSE data-provider issue (not a code defect). The Kite session verified at 20:45 IST tonight does not guarantee readiness at 09:15 IST tomorrow — Kite sessions expire and must be re-confirmed before the first scan fires. Full READY status is only confirmed after the 09:00–09:10 IST pre-market checks pass (see Section 15).
...
| Stale | 0 |
| Scans today | **18** *(in-memory counter since last server restart; a full NSE session runs ~75 scans at 5-min cadence — 77 scans on 2026-08-18 was normal and near-expected)* |
| Cadence | 5 minutes |
...
Scan sessions: **~75 scans per full trading session** (09:15–15:30 IST = 375 min ÷ 5-min cadence; gaps from scan lock contention may reduce this slightly — 77 scans on 2026-08-18 was normal and near-expected). The `scan_count_today` value returned by the API resets on server restart and may show a lower in-session count than the true total.
...
- [ ] **Verify Kite session is live** — `GET /api/phase20/bootstrap-status` → `kite_session_verified: true`
  - ⚠️ The Kite session verified at 20:45 IST the previous evening **does not guarantee next-session readiness**. Zerodha sessions expire daily. If `kite_session_verified: false`, re-authenticate Kite before the first scan fires or the LTP overlay will not apply and bootstrap entries will be blocked.
...
| Expected scans per full session | **~75** (09:15–15:30 = 375 min ÷ 5-min cadence = 75 ticks; 77 scans on 2026-08-18 was normal and near-expected) |
...
| Check | Status | Notes |
|-------|--------|-------|
| Kite LTP overlay | ⚠️ | `kite_overlay_enabled: true` — **session verified at 20:45 IST; must be re-confirmed at 09:00–09:10 IST before first scan** |
| yfinance | ✅ | `provider_health.connection_status: CONNECTED` |
| Latest scan fresh | ⚠️ | Last scan 15:26 IST — **normal for post-market; first scan runs within 5 min of 09:15 IST** |
| LTIM missing | ⚠️ | 1/51 symbols unavailable — ongoing provider issue, not a code defect |

> **Note on Kite session:** Zerodha sessions expire daily. The `kite_session_verified: true` reading captured at 20:45 IST the prior evening does NOT guarantee the session is valid at 09:15 IST the next morning. Always re-run `GET /api/phase20/bootstrap-status` at 09:00–09:10 IST to confirm `kite_session_verified: true` before the first scan. If false, re-authenticate immediately.
>
...
┌─────────────────────────────────────────────────────────┐
│  ✅  READY WITH WARNINGS                                │
│      (confirmed only after 09:00–09:10 IST checks pass) │
│                                                         │
│  Code/infrastructure: no blockers.                     │
│                                                         │
│  Morning checks required before first scan:             │
│  □ /api/healthz = 200                                  │
│  □ bootstrap-status: kite_session_verified = true      │
│  □ positions = []                                       │
│  □ circuit_breaker tripped = false                     │
│  □ bootstrap_max_order_value = 15000                   │
│  □ eod-status: squareoff_time_ist = "15:20 IST"        │
│                                                         │
│  Warnings (non-blocking if checks pass):               │
│  ⚠️  Kite session must be re-verified at 09:00 IST     │
│  ⚠️  LTIM.NS unavailable (pre-existing provider issue) │
│  ⚠️  Last scan 15:26 IST — normal post-market          │
│                                                         │
│  All safety controls active.                           │
│  No overnight carry. EOD square-off confirmed working. │
...
### Before 09:15 IST (2026-08-19) — Required Morning Checks

> ⚠️ **The READY WITH WARNINGS verdict is only confirmed after all six checks below pass at 09:00–09:10 IST.** Do not assume overnight status carries over. Kite sessions expire and must be re-verified every morning.

| # | Action | Expected Result | If Fails |
|---|--------|----------------|----------|
| 1 | `GET /api/healthz` | `{"status":"ok"}` | Restart API server workflow; check logs |
| 2 | `GET /api/phase20/bootstrap-status` → **`kite_session_verified`** | `true` | **Re-authenticate Kite immediately** — without this, LTP overlay is off and bootstrap entries are blocked |
| 3 | `GET /api/phase20/positions` | `[]` | Investigate any unexpected OPEN position before first scan |
| 4 | `GET /api/phase20/circuit-breaker` → **`tripped`** | `false` | Review and manually resume if warranted |
| 5 | `GET /api/phase20/bootstrap-status` → **`bootstrap_max_order_value`** | `15000` | Code issue — escalate; do not trade |
| 6 | `GET /api/phase20/eod-status` → **`squareoff_time_ist`** | `"15:20 IST"` | EOD fix not live — republish before trading |
| 7 | Confirm no live orders | `LIVE_EXECUTION_ENABLED = false` (hardcoded constant — no action needed) | — |

**Production is already published** — no re-publish needed unless new code was merged overnight.
...
*v6.1 — corrections: scan count fixed to ~75/session; Kite re-verification requirement added; morning-checks gate added to READY verdict.*
