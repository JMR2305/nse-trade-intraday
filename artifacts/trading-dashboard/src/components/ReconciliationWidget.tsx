 ...
 *  - Open discrepancies requiring manual review
 *  - Manual trigger button (with force option)
 *  - Per-discrepancy resolve action
 */
...
export default function ReconciliationWidget() {
  ...
  // ── Resolve discrepancy ───────────────────────────────────────────────────
  const resolveOne = useCallback(async (id: number) => {
    setResolvingId(id);
    try {
  ...
  const openDisc = data?.open_discrepancies ?? [];
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {openDisc.map((d: any) => (
                <div key={d.id}
                  className={cn(
                    "rounded-md border px-3 py-2.5 text-xs font-mono",
