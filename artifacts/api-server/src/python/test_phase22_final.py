...
def test_phase22_finalization():
    ...
    try:
        ...
        t_busy = time.time()
        # Lock behavior is independent of durable-universe bootstrap. Supply an
        # explicit authority fixture and fail immediately if any scan is attempted.
        context = {"natural_session": "2026-06-01", "universe_id": 3,
                   "version": 1, "exact_set_hash": "task974-lock-test"}
        with patch("runtime_universe.resolve_active_universe", return_value=context), \
             patch.object(lse, "load_cached_scan", return_value=None), \
             patch.object(lse, "run_live_scan", side_effect=AssertionError("busy lock must not scan")):
    ...
    assert FAIL == 0, f"{FAIL} Phase22 finalization checks failed"


if __name__ == "__main__":
    test_phase22_finalization()
