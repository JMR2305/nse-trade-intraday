...
def _rank_by_opportunity_score(
    universe: list[str], selection_date: str, capital: float,
) -> list[tuple[str, float, str]]:
    """
    Reuses the already lookahead-safe Market Replay per-stock signal
    computation (opportunity score as of selection_date) for ranking.
    ...
    for sym in universe:
        item = replay_stock(sym, selection_date, holding_period=1, interval="daily", capital=capital)
...
def _rank_by_sector_strength(
    universe: list[str], selection_date: str, capital: float, num_stocks: int,
) -> list[tuple[str, float, str]]:
    """
    Computes opportunity score per stock (as-of selection_date, lookahead
    safe), groups by sector, ranks sectors by average score, then returns
    stocks ordered sector-by-sector (strongest sector first, best stocks
    within each sector first) until enough candidates are gathered.
    """
...
def run_paper_basket(
    ...
    # Rank a superset of candidates so we can skip any with unusable data
    # and still land on `num_stocks` simulated items.
    if method == "opportunity_score":
        ranked = _rank_by_opportunity_score(universe, selection_date, capital)
    elif method == "gainers":
        ...
        ranked = _rank_by_sector_strength(universe, selection_date, capital, num_stocks)
    ...
    # Determine the actual next-trading-day buy date (first item's buy_date,
    # since all items share the same universe/date window).
