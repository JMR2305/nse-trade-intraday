...
Runs against explicit cached scan/market/state fixtures (read-only) plus
...
def test_phase9_cached_fixture():
    """Run all legacy assertions with deterministic, isolated input files."""
    global PASS, FAIL, FAILURES
    PASS = FAIL = 0
    FAILURES = []
    with tempfile.TemporaryDirectory(prefix="phase9_test_") as directory:
        names = ("SCAN_CACHE", "MARKET_CACHE", "STATE_FILE", "WATCHLIST_FILE",
                 "ALERTS_FILE", "CONF_HISTORY_FILE", "EXPORT_DIR")
        paths = {name: os.path.join(directory, name.lower()) for name in names}
        with patch.multiple(ce, **paths), patch("signals_store.load_watchlist", return_value=["RELIANCE", "TCS"]):
            ce._save(ce.SCAN_CACHE, {
                "scan_id": "task974_cached_scan",
                "snapshot_ts": "2026-06-01T04:00:00Z",
                "recommendations": [
                    {"symbol": "RELIANCE", "final_action": "BUY",
                     "calibrated_confidence": 75, "confidence": 75,
                     "opportunity_score": 85, "technical_score": 80,
                     "all_gates_passed": True, "rr_ratio": 2.5},
                    {"symbol": "TCS", "final_action": "IGNORE",
                     "calibrated_confidence": 30, "confidence": 30,
                     "opportunity_score": 20, "technical_score": 25,
                     "all_gates_passed": False, "rr_ratio": 0.5},
                ],
            })
            ce._save(ce.MARKET_CACHE, {})
            ce._save(ce.STATE_FILE, {"cash": 5000, "positions": [], "trades": []})
            ce._save(ce.WATCHLIST_FILE, ["RELIANCE", "TCS"])
            _run_checks()
