...
# Task978ZC permits only the reviewed standalone-bootstrap completeness
# correction. The implicit-transaction durability fix in the bootstrap and its
# dependency-closure/durability regressions are pinned as exact regular-file
# blobs: the historical Task978ZA reviewed blob must remain at the reviewed
# commit, and the exact corrected blob must be present at validation HEAD.
# Path-only or future-content trust is intentionally prohibited.
TASK978ZC_REVIEWED_BLOBS = {
    'scripts/task978r_clean_authority.py':
        'f8395c02e37eec46ff96a947d5d3e693a599fd82',
    'scripts/test_task978r_clean_authority.py':
...
# Task978ZN-R7 corrects only the native validator's proof fixture: order
# independence is exercised deterministically rather than relying on a
# particular ICU build to disagree with Python ordering. Preserve the entire
# R6 layer above at its reviewed commit and permit only these two exact regular
# file blobs to supersede it at validation HEAD.
TASK978ZN_R7_REVIEWED_COMMIT = '0dc539886e60b2e5be2035a2de8ba31a3265fdd2'
TASK978ZN_R7_REVIEWED_BLOBS = {
    'scripts/task978za_postgres_validation.py':
        '654e6667432163a3180fe1b8cb22c9967f93916b',
    'scripts/test_task978za_postgres_validation.py':
        'd54927b1bf704f15fa844aec398d1533c7cb1acb',
}
# Task978ZQ authorizes only the reviewed canonical current-price incident
# authority correction and its exact regression. Require the reviewed repair
# commit in ancestry and both exact regular-file blobs at validation HEAD.
TASK978ZQ_REVIEWED_COMMIT = '5f699fabf39ca524f4b4e45e42919472018caece'
TASK978ZQ_REVIEWED_BLOBS = {
    'artifacts/api-server/src/python/market_data_incidents.py':
        'b5d4ac79a3b769a43140989a0b206711896bebe9',
...
def identity():
    ...
    for path, r7_blob in TASK978ZN_R7_REVIEWED_BLOBS.items():
        expected_entry = f'100644 blob {r7_blob}\t{path}'
        if git('ls-tree', TASK978ZN_R7_REVIEWED_COMMIT, '--', path) != expected_entry:
            raise RuntimeError(f'Unexpected Task978ZN-R7 reviewed content: {path}')
        if git('ls-tree', head, '--', path) != expected_entry:
            raise RuntimeError(f'Unexpected Task978ZN-R7 candidate content: {path}')
    if TASK978ZQ_REVIEWED_COMMIT not in git('rev-list', 'HEAD').splitlines():
        raise RuntimeError('Task978ZQ reviewed commit absent from ancestry')
    for path, zq_blob in TASK978ZQ_REVIEWED_BLOBS.items():
        expected_entry = f'100644 blob {zq_blob}\t{path}'
        if git('ls-tree', TASK978ZQ_REVIEWED_COMMIT, '--', path) != expected_entry:
    ...
    proof = {'workflow_head': head, 'reviewed_ancestor': ancestor,
             ...
             'task978zn_r7_exact_validator_blobs': {
                 'reviewed_commit': TASK978ZN_R7_REVIEWED_COMMIT,
                 'blobs': TASK978ZN_R7_REVIEWED_BLOBS,
             },
